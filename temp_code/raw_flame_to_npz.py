...
eye_pose is stored separately as an extra key (useful for gaze rendering):
  eye_pose: (1, N,   6)  float32
...
def convert(input_dir: str, output_path: str):
    ...
    for fd in frame_dirs:
        ...
        eye_pose_list.append(np.load(fd / 'eye_pose.npy'))
    ...
    eye_pose = np.stack(eye_pose_list, axis=0)[np.newaxis]  # (1, N,   6)

    print(f"  shape:    {shape.shape}  dtype={shape.dtype}")
    print(f"  exp:      {exp.shape}  dtype={exp.dtype}")
    print(f"  pose:     {pose.shape}  dtype={pose.dtype}")
    print(f"  eye_pose: {eye_pose.shape}  dtype={eye_pose.dtype}")

    np.savez(output_path, shape=shape, exp=exp, pose=pose, eye_pose=eye_pose)
